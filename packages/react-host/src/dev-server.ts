/* eslint-disable @typescript-eslint/no-var-requires */
/* eslint-disable global-require */
/* eslint-disable no-console */
import chalk from 'chalk';
import ejs from 'ejs';
import express from 'express';
import path from 'path';

// Development only. In every deployed environment the registry renders the shell
// and inlines the composition; this exists to iterate on the host bundle itself.
const port = process.env.APPSHELL_PORT || 9000;
const app = express();

const webpack = require('webpack');
const webpackDevMiddleware = require('webpack-dev-middleware');
const webpackHotMiddleware = require('webpack-hot-middleware');
const webpackConfig = require('../webpack.config');

const compiler = webpack(webpackConfig(null, { mode: 'development' }));

app.use(webpackDevMiddleware(compiler, { publicPath: 'auto' }));
app.use(webpackHotMiddleware(compiler));

const registry = process.env.APPSHELL_REGISTRY || './appshell_registry';
console.log(`Serving static from ${registry}`);
app.use(express.static(registry));
app.use(express.static(__dirname));

app.engine('.html', ejs.renderFile);
app.set('views', path.join(__dirname, 'views'));
app.set('view engine', 'html');

app.get('*', (req, res) => {
  res.render('index', {
    favIcon: process.env.APPSHELL_FAVICON || '/favicon.ico',
    title: process.env.APPSHELL_TITLE || 'Appshell',
    description: process.env.APPSHELL_DESCRIPTION || 'Appshell is awesome!',
    publicUrl: path.join(process.env.APPSHELL_PUBLIC_URL || '', '/'),
    themeColor: process.env.APPSHELL_THEME_COLOR,
    stylesheetUrl: process.env.APPSHELL_STYLESHEET_URL,
  });
});

app.listen(port, () => {
  console.log(chalk.cyan(`Appshell host dev server listening on port ${port}`));
});
